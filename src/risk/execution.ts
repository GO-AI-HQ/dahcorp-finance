...
export function validateRobinhoodExecution(input: LiveExecutionGateInput): LiveExecutionGateResult {
  ...
  if (input.killSwitch) findings.push(block('KILL_SWITCH', 'The global kill switch is engaged. No live order may be submitted.'));
  if (symbol !== ROBINHOOD_LIVE_EXECUTION_SYMBOL) findings.push(block('SYMBOL_NOT_ALLOWLISTED', `Robinhood execution is restricted to ${ROBINHOOD_LIVE_EXECUTION_SYMBOL}.`));
  if (input.side !== 'buy') findings.push(block('BUY_ONLY', 'The Robinhood live execution policy permits buys only.'));
  if (input.orderType !== 'market') findings.push(block('MARKET_ONLY', 'The Robinhood live execution policy permits market orders only.'));
  if (!Number.isInteger(input.quantity) || input.quantity <= 0) findings.push(block('WHOLE_SHARES_REQUIRED', 'The first Robinhood execution surface is limited to positive whole-share quantities.'));
