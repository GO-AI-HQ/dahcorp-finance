...
function parseBudgetValue(value: unknown, day: string, limit: number): FmpBudgetStatus {
  ...
  return {
    ...
    note: used >= limit
      ...
      : `DAHCorp has ${Math.max(0, limit - used)} FMP call${Math.max(0, limit - used) === 1 ? '' : 's'} left in its ${limit}-call safety budget for ${day}.`,
  };
}
...
export async function getFmpDailyBudgetStatus(env?: RuntimeEnv, now = new Date()): Promise<FmpBudgetStatus> {
  ...
  if (!db) {
    return {
      ...
      remaining: 0,
      available: false,
      note: 'The persistent budget store is unavailable, so DAHCorp will not make FMP network calls.',
    };
  }
  try {
    ...
    return {
      ...
      remaining: 0,
      available: false,
      note: 'The FMP safety budget could not be verified, so DAHCorp will not make an FMP network call.',
    };
  }
}
...
export async function reserveFmpCall(purpose: string, env?: RuntimeEnv, now = new Date()): Promise<FmpBudgetStatus> {
  ...
  const limit = fmpDailyBudgetLimit(env);
  const db = getDb();
  if (!db) return getFmpDailyBudgetStatus(env, now);

  const key = `${BUDGET_KEY_PREFIX}${day}`;
  const initialValue = JSON.stringify({ day, used: 1, limit, lastPurpose: purpose.slice(0, 80) });
  try {
    const result = await db.execute(sql`
      ...
      SET "value" = jsonb_build_object(
            ...
            'limit', ${limit},
            'lastPurpose', ${purpose.slice(0, 80)}
          ),
          "updated_at" = NOW()
    ...
    if (rows.length) return parseBudgetValue(rows[0]?.value, day, limit);
    ...
    return {
      ...
      remaining: 0,
      available: false,
      note: 'The FMP safety budget could not reserve a call, so DAHCorp blocked the provider request.',
    };
  }
}
