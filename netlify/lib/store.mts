 ...
 * labelled seed model and the audit writes become no-ops that are logged. That
...
export async function loadStrategyConfig(): Promise<LoadedConfig> {
  ...
  if (!db) {
    return {
      ...
      persisted: false,
      note: 'No database attached — strategy settings are the defaults and changes will not persist.',
    };
  }
  try {
    ...
    return {
      config: DEFAULT_STRATEGY_CONFIG,
      persisted: false,
      note: 'Strategy settings could not be read. Defaults are in effect.',
    };
  }
}
...
export async function saveStrategyConfig(patch: Partial<StrategyConfig>): Promise<LoadedConfig> {
  ...
  await db
    ...
    .values({ key: STRATEGY_KEY, value: next })
...
export async function loadPositionSource(asOf: string): Promise<PositionSource> {
  ...
  try {
    ...
    const incomeEvents: IncomeEvent[] = incomeRows.map((row) => ({
      id: String(row.id),
      accountId: row.accountExternalId,
      symbol: row.symbol.toUpperCase(),
      payDate: row.payDate,
      grossAmount: row.grossAmount,
      sharesAtRecord: row.sharesAtRecord,
      reinvested: row.reinvested,
    }));

    const contributions: Contribution[] = contributionRows.map((row) => ({
      id: String(row.id),
      accountId: row.accountExternalId,
      date: row.date,
      amount: row.amount,
    ...
    const corporateActions: CorporateAction[] = actionRows.map((row) => ({
      symbol: row.symbol.toUpperCase(),
      effectiveDate: row.effectiveDate,
      type: row.type as CorporateAction['type'],
      ratio: row.ratio ?? undefined,
      newSymbol: row.newSymbol ?? undefined,
    ...
    return {
      ...
      accounts,
      holdings,
      incomeEvents: incomeEvents.length ? incomeEvents : null,
      contributions,
      corporateActions,
      notes: mock
        ? ['Positions are stored records flagged as mock data, not live brokerage data.']
        : ['Positions are stored records entered by the investor.'],
      containsMockData: mock,
    };
  } catch (error) {
    ...
    return {
      ...fallback,
      notes: [...fallback.notes, 'Stored positions could not be read; showing the seeded mock model.'],
    };
  }
}

export interface AuditEntry {
  category: 'auth' | 'config' | 'agent' | 'risk' | 'order' | 'data' | 'error';
  action: string;
...
export async function recordAudit(entry: AuditEntry): Promise<void> {
  const db = getDb();
  if (!db) {
    console.log(`[dahcorp][audit:${entry.category}] ${entry.action} — ${entry.message ?? ''}`);
    return;
  }
  try {
    await db.insert(schema.auditLog).values({
      ...
      detail: entry.detail === undefined ? null : (entry.detail as object),
    });
  } catch (error) {
    console.error('[dahcorp] recordAudit failed:', error);
  }
}
...
export async function listAuditLog(limit = 100) {
  ...
  if (!db) return [];
  try {
    return await db.select().from(schema.auditLog).orderBy(desc(schema.auditLog.createdAt)).limit(limit);
  } catch (error) {
    console.error('[dahcorp] listAuditLog failed:', error);
    return [];
  }
}

/**
 * Income snapshots exist so Income Velocity can separate market drift from
 * contributions and DRIP. Without a prior measurement the market component is
 * reported as unavailable rather than guessed.
 */
export async function recordIncomeSnapshot(row: {
  asOf: string;
  ...
  try {
    await db
      .insert(schema.incomeSnapshots)
      .values(row)
      .onConflictDoUpdate({
        target: schema.incomeSnapshots.asOf,
        set: {
          forwardMonthlyIncome: row.forwardMonthlyIncome,
          incomeEngineCapital: row.incomeEngineCapital,
          blendedDistributionRate: row.blendedDistributionRate,
          portfolioValue: row.portfolioValue,
          basis: row.basis,
          containsMockData: row.containsMockData,
        },
      });
  } catch (error) {
    console.error('[dahcorp] recordIncomeSnapshot failed:', error);
  }
}

/** Most recent snapshot strictly before `asOf`, for the velocity residual. */
export async function priorIncomeSnapshot(asOf: string) {
  ...
  try {
    const rows = await db
      .select()
      .from(schema.incomeSnapshots)
      .where(lt(schema.incomeSnapshots.asOf, asOf))
      .orderBy(desc(schema.incomeSnapshots.asOf))
      .limit(1);
    return rows[0] ?? null;
  } catch (error) {
    console.error('[dahcorp] priorIncomeSnapshot failed:', error);
    return null;
  }
}
  ...
  try {
    const rows = await db
      ...
      .values({
        ...
        availableCapital: record.availableCapital,
        source: record.source,
        model: record.model,
        headline: record.headline,
        confidence: record.confidence,
        brief: record.brief as object,
        portfolioSnapshot: record.portfolioSnapshot as object,
        deterministicOutcome: record.deterministicOutcome as object,
      })
      .returning({ id: schema.recommendations.id });
    return rows[0]?.id ?? null;
  } catch (error) {
    console.error('[dahcorp] saveRecommendation failed:', error);
    return null;
  }
}
...
export async function listRecommendations(limit = 25) {
  ...
  if (!db) return [];
  try {
    return await db.select().from(schema.recommendations).orderBy(desc(schema.recommendations.createdAt)).limit(limit);
  } catch (error) {
    console.error('[dahcorp] listRecommendations failed:', error);
    return [];
  }
}
...
export async function setRecommendationAction(
  ...
  try {
    const rows = await db
      ...
      .set({ userAction, userNote, actedAt: new Date() })
      .where(and(eq(schema.recommendations.id, id), eq(schema.recommendations.userAction, 'pending')))
      .returning({ id: schema.recommendations.id });
    return rows.length > 0;
  } catch (error) {
    console.error('[dahcorp] setRecommendationAction failed:', error);
    return false;
  }
}
...
export async function listOrderPreviews(limit = 25) {
  ...
  if (!db) return [];
  try {
    return await db.select().from(schema.orderPreviews).orderBy(desc(schema.orderPreviews.createdAt)).limit(limit);
  } catch (error) {
    console.error('[dahcorp] listOrderPreviews failed:', error);
    return [];
  }
}
